...
import dungeonmania.entities.logicSwitches.LogicalEntities.LightBulb;
import dungeonmania.entities.logicSwitches.LogicalEntities.SwitchDoor;
import dungeonmania.entities.logicSwitches.LogicalRules.And;
import dungeonmania.entities.logicSwitches.LogicalRules.CoAnd;
import dungeonmania.entities.logicSwitches.LogicalRules.LogicRule;
import dungeonmania.entities.logicSwitches.LogicalRules.Or;
import dungeonmania.entities.logicSwitches.LogicalRules.Xor;
...
public class EntityFactory {
    ...
    public void spawnSpider(Game game) {
        ...
        Position initPosition = game.getPositionsForSpider(dummySpider, ranGen);
    ...
    public void spawnZombie(Game game, ZombieToastSpawner spawner) {
        ...
        List<Position> pos = spawner.getPositionsForZombie(map);
    ...
    public LightBulb buildLightBulb(Position pos, String logic) {
        LogicRule logicRule = buildLogic(logic);
        return new LightBulb(pos, logicRule);
    }

    public SwitchDoor buildSwitchDoor(Position pos, String logic) {
        LogicRule logicRule = buildLogic(logic);
        return new SwitchDoor(pos, logicRule);
    }

    public LogicRule buildLogic(String logic) {
        switch (logic) {
            case "and":
                return new And();
            case "or":
    ...
    private Entity constructEntity(JSONObject jsonEntity, JSONObject config) {
        ...
        case "light_bulb_off":
            return buildLightBulb(pos, jsonEntity.getString("logic"));
        case "switch_door":
            return buildSwitchDoor(pos, jsonEntity.getString("logic"));
        case "wire":
            return new Wire(pos);
        case "swamp_tile":
