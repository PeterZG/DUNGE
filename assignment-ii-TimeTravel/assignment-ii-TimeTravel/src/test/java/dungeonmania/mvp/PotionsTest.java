...
public class PotionsTest {
    ...
    public void invincibilityDuration() throws InvalidActionException {
        ...
        assertEquals(1, TestUtils.getEntities(res, "spider").size());
        res = dmc.tick(Direction.DOWN);
        List<RoundResponse> round2 = dmc.getDungeonResponseModel().getBattles().get(1).getRounds();
        assertEquals(-0.5, round2.get(1).getDeltaCharacterHealth(), 0.001);
        assertEquals(-2, round2.get(1).getDeltaEnemyHealth(), 0.001);
