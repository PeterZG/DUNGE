...
public class BouldersTest {
    ...
    @Test
    @Tag("3-4")
    @DisplayName("Test the notPlayer is unable to push the boulders")
    public void notPlayer() {
        DungeonManiaController dmc = new DungeonManiaController();
        DungeonResponse res = dmc.newGame("d-BoulderTest_notPlayer", "c_mercenaryTest_simpleMovement");
        ...
        * 3                    w
        */
        assertTrue(boulderAt(res, 4, 2));

        // Player moves boulder
        res = dmc.tick(Direction.LEFT);
        assertTrue(boulderAt(res, 4, 2));
        assertEquals(new Position(1, 2), TestUtils.getPlayer(res).get().getPosition());
    }
}
